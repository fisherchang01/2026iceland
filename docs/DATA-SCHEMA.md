...
### 3.1 導航目的地（navTargets）

行程頁「距離/時間 + Google/Apple 導航」按鈕已經改成**附掛在景點卡自己身上**（跟景點卡是同一個外框、同一塊底色），不再是卡片下方另外飄一條獨立的時間軸列。

`navTargets` 是一個陣列，每一筆格式：

```js
{ name: '超市', map: 'Bónus Selfoss Iceland', type: 'drive', dist: '15 km', time: '15 分钟' }
```

- `name`：顯示名稱
- `map`：地圖查詢字串／地址（沒填就退回用 `name` 查詢）
- `type`：`drive`（預設）／`walk`／`tram`，決定圖示與底色
- `dist`、`time`：選填，顯示在名稱後面（字面顯示，不會自動計算）

只填一筆時，行程頁只顯示一組導航按鈕，跟改版前一樣單純；填兩筆以上時，行程頁會自動在每一筆前面加上**這個景點自己的字母＋序號**（例如景點 A 有兩筆就是 A1、A2）。

沒有填 `navTargets` 時，完全維持向下相容的舊行為，不用改任何既有資料：

1. 有 `nextStops`（既有的多段鏈式導航，例如 Kerið→超市→民宿）→ 沿用，效果等同於填了多筆 `navTargets`
2. 有 `drives[i]`（當天 `drives` 陣列，自駕距離/時間）→ 目的地自動抓「下一個主景點」
3. 有 `nextStop`（單一步行/輕軌/自駕備註）→ 目的地自動抓「下一個主景點」

`nextStops`／`drives`／`nextStop` 目前**只能手動改資料檔**，尚未接上行程編輯器的 GUI；`navTargets` 是唯一能在編輯器裡新增/修改的欄位，之後新增導航目的地建議直接用這個欄位。
  ...
  categories: [
    {
      ...
      items: [
        {
          id: "skyr",           // 唯一識別碼（v1.1 起必填），照片檔名的前綴，見下方說明
...
- 格式限制：**小寫英文字母、數字與減號**，且以英數開頭（`^[a-z0-9][a-z0-9-]*$`）
- 唯一性：**跨兩個資料檔全域唯一**。`images/catalog/` 是體驗頁與工具頁共用的單一扁平資料夾，兩個項目用同一個 id 會共用同一組流水號
- 上傳資料檔前會被 Guard 檢查（非空、格式、本檔內不重複）；與另一頁撞名只會警告，不阻擋
- 改 id **不會**動到已經存在的檔案，只影響之後新上傳的命名。舊照片留著原本的名字是正常的

渲染端（`js/render-travel.js`、`js/catalog-nav.js`）完全不讀這個欄位，它只服務編輯器。
