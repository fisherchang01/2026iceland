...
- `cover-hero.webp`（體驗／工具頁 Hero 大圖）：顯示區塊高度約 330px、寬度＝裝置寬，建議比例約 **1.2:1～1.3:1**（例如 1200×960px 或 1400×1100px），WebP 品質 80–85
- 這兩種都是 `background-size:cover` 置中裁切，不需要跟景點照片一樣分兩個尺寸

體驗／工具目錄圖片（`images/catalog/`）：

- 同一張圖可能同時被套用在三種不同比例的容器（滿版卡 2.2:1 橫幅／左圖右文卡約 0.9:1 接近方形／詳情 Sheet 約 1.8:1 橫幅），與其分別裁三個版本，**建議原始素材就抓「主體置中、四周留安全邊距」的接近方形構圖（比例約 1:1～1.2:1）**，這樣不管被套進哪一種容器裁切，重點都不會被切掉
- 建議至少準備 800×800px 以上的原圖，統一轉 WebP
