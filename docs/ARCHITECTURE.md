...
`js/catalog-nav.js` 曾因「使用者可見文字統一為簡體」被改過 17 個字。判斷依據：
變更性質是**純顯示文字**，未動選擇器、DOM 契約、函式或邏輯，`git diff --numstat`
為等量增減行（逐行字元替換）。這是允許的例外類型——**改的是字面，不是行為**。
不要把它當成「第一層可以動」的先例。

`css/style.css` 有 42 個確認的死碼 class（約 58 條規則、7 KB），
**刻意保留不刪**，清單與掃描依據見 [CSS-DEAD-CODE.md](CSS-DEAD-CODE.md)。
在使用中的行程站上動這個檔案，風險大於收益；要清請在複製成新旅程範本時，
在新的副本上做。
