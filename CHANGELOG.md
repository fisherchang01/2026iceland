...
1. `style.css` 是 §9 的**第一層不可動區**，v1.3 已經為 `.fs-*` 做過同樣的判斷
2. 收益只有 7 KB / 79 KB，而這是一個**正在使用中、十月要帶去冰島用**的行程站
3. 這個 repo 的定位是「之後美東、泰國行程的範本」——清理的正確時機是**複製出新副本的時候**，不是現在

**掃描時發現的關鍵細節：第一次靜態掃描報 44 個死碼，其中 2 個是誤判。**

`ov-ch-0` / `ov-ch-2` 在 `js/render-overview.js` 是用 `'ov-ch-' + chapterIdx` 串接出來的，靜態搜尋找不到，但它們控制總覽頁時間軸的章節配色——照掃描結果刪掉會直接弄壞首頁。同類的還有 `catalog-nav.js:198` 的 `'catalog-layout-' + layout`。

全站只有這兩處是字串串接組 class，其餘一律是字面值。這件事本身值得記下來，因為它是「不能只靠搜尋不到就刪」的具體證據。

`docs/CSS-DEAD-CODE.md` 記錄了完整清單、掃描方法、每一組的由來，以及清理時要目視檢查哪些頁面。`ARCHITECTURE.md` §9 加了「已知例外」小節指向它。
